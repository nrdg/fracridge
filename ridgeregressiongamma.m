...
% where p and/or b are large. For small problems, we may
% take a hit in computational time due to various
% overhead-related steps.
%
...
% rotate the data
ynew = u'*y;  % p x b (OR n x b)

% compute the OLS solution in the rotated space
hols_rotatedspace = bsxfun(@rdivide,ynew,selt);  % p x b (OR n x b)
hols_rotatedspace(isbad,:) = 0;  % the solution will be 0 for eigenvalues that are essentially zero (pseudoinverse)
...
% pre-compute for speed
scLG = bsxfun(@rdivide,seltSQ,bsxfun(@plus,seltSQ,lambdagrid));        % p x lambdagrid (OR n x lambdagrid)
...
for ii=1:b
  ...
  % compute vector length for each lambda
    % OLD SLOW WAY: len = vectorlength(bsxfun(@times,scLG,hols_rotatedspace(:,ii)),1);   % 1 x lambdagrid
  len = sqrt((scLG.^2'*hols_rotatedspace(:,ii).^2)');                     % 1 x lambdagrid
  ...
  assert(min(len) < 0.0001);                      % if all is well, we should have sampled close to 0
  ...
  % get the solutions
  sc = bsxfun(@rdivide,seltSQ,bsxfun(@plus,seltSQ,temp));              % p x f (OR n x f)
  sc(isbad,:) = 0;                                                     % for safety, ensure bad eigenvalues get scalings of 0
  temp = v*bsxfun(@times,sc,hols_rotatedspace(:,ii));                  % rotate solution to original space
  h(:,:,ii) = temp;
